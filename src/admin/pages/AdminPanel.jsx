...
const emptyForm = () => ({
  name: "", category: "food", image: "", description: "",
  variants: [{ name: "", price: "" }],
});
...
const ImageUploader = ({ value, onChange }) => {
  ...
  const uploadFile = async (file) => {
    ...
    if (!allowed.includes(file.type)) { setError("Chỉ chấp nhận ảnh JPG, PNG, WebP, GIF"); return; }
    ...
    try {
      ...
      if (data.success) onChange(data.url); else setError(data.message || "Upload thất bại");
    } catch { setError("Lỗi kết nối khi upload"); }
    ...
    return (
      <div className="img-up-preview">
        ...
        <div className="img-up-actions">
          <button type="button" className="img-up-change" onClick={() => fileRef.current?.click()}>🔄 Đổi ảnh</button>
          <button type="button" className="img-up-remove" onClick={handleRemove}>✕ Xóa</button>
        </div>
  ...
  return (
    <div>
      <div className={`img-up-dropzone ${dragOver ? "drag-over" : ""} ${uploading ? "uploading" : ""}`}
        onClick={() => !uploading && fileRef.current?.click()}
        onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
        onDragLeave={() => setDragOver(false)} onDrop={handleDrop}>
        {uploading ? (
          <div className="img-up-loading"><span className="adm-spinner" /><span>Đang upload...</span></div>
        ) : (
...
const ProductModal = ({ product, onSave, onClose, saving }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (form.variants.some((v) => !v.name.trim() || v.price === "")) return setErr("Tất cả biến thể phải có tên và giá.");
    setErr("");
    onSave({ ...form, variants: form.variants.map((v) => ({ name: v.name.trim(), price: parseInt(v.price), inventory_item_id: v.inventory_item_id || undefined })) });
  };
    ...
    <div className="adm-modal-overlay" onClick={onClose}>
      <div className="adm-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form className="adm-modal-body" onSubmit={handleSubmit}>
          ...
          {/* ── VariantEditor mới — truyền token để import từ kho ── */}
          <VariantEditor
  ...
  <div className="adm-modal-overlay" onClick={onClose}>
    <div className="adm-modal adm-modal-sm" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="adm-modal-body">
        <p className="adm-delete-msg">Bạn có chắc muốn xóa sản phẩm <strong>"{product.name}"</strong>?<br />Hành động này không thể hoàn tác.</p>
        <div className="adm-modal-actions">
...
const AdminPanel = () => {
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      const res = isEdit ? await adminAPI.updateProduct(formData.id, formData) : await adminAPI.createProduct(formData);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.error) throw new Error(res.error);
      showToast("Đã xóa sản phẩm!"); setModal(null); fetchProducts();
  ...
  return (
    <div>
      ...
      <div className="adm-filters">
        <input className="adm-input adm-search" placeholder="🔍 Tìm sản phẩm..." value={search} onChange={(e) => setSearch(e.target.value)} />
        <div className="adm-cat-tabs">
          <button className={`adm-cat-tab ${catFilter === "all" ? "active" : ""}`} onClick={() => setCatFilter("all")}>Tất cả</button>
          {CATEGORIES.map((c) => (
            <button key={c.id} className={`adm-cat-tab ${catFilter === c.id ? "active" : ""}`} onClick={() => setCatFilter(c.id)}>{c.label}</button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="adm-empty"><div className="adm-empty-icon">📦</div><p>Không có sản phẩm nào</p></div>
      ) : (
        <div className="adm-table-wrap">
          <table className="adm-table">
            <thead>
              <tr><th>ID</th><th>Ảnh</th><th>Tên sản phẩm</th><th>Danh mục</th><th>Số variant</th><th>Giá từ</th><th>Thao tác</th></tr>
            </thead>
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id}>
                  <td className="adm-td-id">#{p.id}</td>
                  <td>{p.image ? <img className="adm-thumb" src={p.image} alt={p.name} /> : <div className="adm-thumb adm-thumb-empty">📷</div>}</td>
                  <td>
                    <div className="adm-product-name">{p.name}</div>
                    <div className="adm-product-desc">{p.description}</div>
                  </td>
                  <td><span className={`adm-badge adm-badge-${p.category}`}>{CAT_LABEL[p.category] || p.category}</span></td>
                  <td className="adm-td-center">{p.variants.length}</td>
                  <td className="adm-td-price">{Math.min(...p.variants.map((v) => v.price)).toLocaleString("vi-VN")}đ</td>
                  <td>
                    <div className="adm-actions">
                      <button className="adm-action-btn adm-edit" onClick={() => setModal({ mode: "edit", product: p })}>✏️ Sửa</button>
                      <button className="adm-action-btn adm-delete" onClick={() => setModal({ mode: "delete", product: p })}>🗑️ Xóa</button>
                    </div>
                  </td>
                </tr>
              ))}
