  ...
  return (
    <div className="sl-line">
      <div className="sl-line-product">
        <select
          ...
          onChange={(e) => {
            ...
            onChange({ ...item, productId: pid, productName: p?.name || "", variantName: p?.variants[0]?.name || "", price: p?.variants[0]?.price || 0, qty: 1 });
          }}
        >
      ...
      <div className="sl-line-qty">
        ...
        <input className="sl-qty-input" type="number" min="1" value={item.qty}
      ...
      <div className="sl-line-price">
        <input className="sl-price-input" type="number" min="0" step="1000" value={item.price}
...
const AdminSales = () => {
  ...
  const addLine = () => {
    ...
    setLines((prev) => [...prev, {
      id: Date.now(), productId: first?.id || null, productName: first?.name || "",
      variantName: first?.variants[0]?.name || "", price: first?.variants[0]?.price || 0, qty: 1,
    }]);
  };
  ...
  const handlePrint = async () => {
    ...
    try {
      const payload = {
        ...
        items: lines.map((l) => ({ product_id: l.productId, product_name: l.productName, variant_name: l.variantName, price: l.price, qty: l.qty })),
        ship_fee: shipFee, discount, note: note.trim(),
      };
      ...
      const invoiceData = {
        invoiceNo: result.invoice_no, orderId: result.order_id,
        createdAt: new Date().toLocaleString("vi-VN"), customer,
        lines: lines.map((l) => ({ productName: l.productName, variantName: l.variantName, qty: l.qty, price: l.price, subtotal: l.price * l.qty })),
  ...
  return (
    <div>
      <div className="adm-topbar">
        ...
        <button className="adm-btn-primary" disabled={!canInvoice || saving} onClick={handlePrint}>
        ...
        <div className="sl-left">
          <div className="sl-card">
            <div className="sl-card-header">
              ...
              <button className="adm-btn-primary sl-add-btn" onClick={addLine} disabled={loading}>+ Thêm dòng</button>
            </div>
            {lines.length === 0 ? (
              ...
              <>
                ...
                {lines.map((l) => (
                  <LineItem key={l.id} products={products} item={l}
                    onChange={(data) => updateLine(l.id, data)} onRemove={() => removeLine(l.id)} />
                ))}
              </>
            )}
          </div>
          <div className="sl-card sl-card-note">
            ...
            <textarea className="sl-textarea" placeholder="Ghi chú thêm (tặng kèm, yêu cầu đặc biệt...)"
        ...
        <div className="sl-right">
          <div className="sl-card">
            ...
            <div className="sl-fields">
              <div className="adm-field">
                ...
                <input className="adm-input" placeholder="Nguyễn Văn A" value={customer.name} onChange={(e) => setCust("name", e.target.value)} />
              </div>
              <div className="adm-field">
                <label className="adm-label">Số điện thoại</label>
                <input className="adm-input" placeholder="0901234567" type="tel" value={customer.phone} onChange={(e) => setCust("phone", e.target.value)} />
              </div>
              <div className="adm-field">
          ...
          <div className="sl-card sl-summary">
            ...
            <div className="sl-sum-rows">
              <div className="sl-sum-row"><span>Tiền hàng</span><span>{fmt(subtotal)}</span></div>
              <div className="sl-sum-row">
                <span>Phí ship</span>
                <div className="sl-sum-input-wrap">
                  <input className="sl-sum-input" type="number" min="0" step="1000" value={shipFee}
              ...
              <div className="sl-sum-row">
                <span>Giảm giá</span>
                <div className="sl-sum-input-wrap">
                  <input className="sl-sum-input" type="number" min="0" step="1000" value={discount}
            ...
            <button className="sl-print-btn" disabled={!canInvoice || saving} onClick={handlePrint}>
              {saving ? "⏳ Đang lưu..." : "🖨️ Xem & In hóa đơn"}
            </button>
            {!canInvoice && !saving && <p className="sl-hint">Điền tên khách và thêm ít nhất 1 sản phẩm</p>}
          </div>
        </div>
      </div>
    </div>
